...
def setup_test_case(request, container_path, softwarecontainer_binary):
    """ Setup fixture that is run once per test function. Sets the config
        for the current test, starts SoftwareContainer and calls create_app().
    """
    global app_bin
...
class TestPulseGateway():
    """ Tests the SoftwareContainer PulseAudio gateway using different configurations.
